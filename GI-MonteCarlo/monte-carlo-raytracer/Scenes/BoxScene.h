...
class BoxScene : public AbstractSceneFactory{
public:
    
    
    static void place_objects(std::shared_ptr< StandardScene > scene){
        try {
            std::vector<std::shared_ptr<Triangle> > triangles = ObjLoad::loadMesh("prism.obj");
            for(std::shared_ptr<Triangle> triangle : triangles){
                //scene->addPrimitive(triangle);
                triangle->multMatrix(Matrix::translate(0.f, -1.5f, 0.f));
                //triangle->multMatrix(Matrix::yrotate(M_PI/2.f));//(2.2f, -3.0f, 1.f));
                //triangle->multMatrix(Matrix::zrotate(M_PI/9.f));//(2.2f, -3.0f, 1.f));
                triangle->multMatrix(Matrix::yrotate(M_PI/2.f));//(2.2f, -3.0f, 1.f));

                triangle->multMatrix(Matrix::scale(0.3f,0.3f,0.3f));
        ...
        std::shared_ptr<Sphere> sphere(new Sphere(1.5f));
        sphere->multMatrix(Matrix::translate(0.f, -2.5f, 0.f));
        //sphere->multMatrix(Matrix::scale(1.f, 0.5f, 1.f));
        sphere->setRefraction(std::shared_ptr<Refraction>(new Cauchy(Cauchy::michoGlass())));
...
public:
	static CUDA_CALLABLE_MEMBER std::shared_ptr< StandardScene > scene(){
        ...
        light->multMatrix(Matrix::scale(CornellBoxScale/2.f));

...
void spectrum_to_xyz(float I[],float w[],int N, float *x, float *y, float *z){
    ...
    for (int j = 0; j < N; j++) {
        assert(w[j] >= 380 && w[j] < 780);

        int i = std::floor((w[j]-380.f)/res);
        float a = w[j]-i*res+380.f;

        X += I[j] * cie_colour_match[i][0]+(cie_colour_match[i+1][0]-cie_colour_match[i][0])*a;
        Y += I[j] * cie_colour_match[i][1]+(cie_colour_match[i+1][1]-cie_colour_match[i][1])*a;
        Z += I[j] * cie_colour_match[i][2]+(cie_colour_match[i+1][2]-cie_colour_match[i][2])*a;

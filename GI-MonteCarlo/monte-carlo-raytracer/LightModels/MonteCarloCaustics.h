#ifndef MonteCarloCaustics_H
#define MonteCarloCaustics_H
...
class MonteCarloCaustics : public LightModel{
protected:
	int direct_samples;
	int indirect_samples;
private:
    //Tree
    KDTree<float, 3,Photon> **caustics_;
public:

	MonteCarloCaustics(const std::shared_ptr<Scene> &scene,
                       KDTree<float, 3,Photon> **caustics,
                       int direct = 1,
                       int indirect = 1):LightModel(scene), caustics_(caustics),direct_samples(direct), indirect_samples(indirect){
    ...
    CUDA_CALLABLE_MEMBER float radianceTransfer(const Vec3f &Nx, const Vec3f &x, const Vec3f &Ny, const Vec3f &y, const Primitive* lightSource,float wavelength, random_kit_t& rnd) const{
        ...
        //V
        float dummy_t;
        float V = (LightModel::scene_->hit(ray,dummy_t) == lightSource);
        //G
        float a = Nx.dot(dir);
    ...
    CUDA_CALLABLE_MEMBER float causticRadiance(const Primitive *p,
                                               const Ray &ray, Vec3f x,
                                               float wavelength,
        ...
        std::multimap<float, Photon > closest =  caustics_[(int)floor((wavelength-380.f)/float(RESOLUTION))]->search_n(x,60, max_distance);
        ...
        for (auto &it : closest) {
            ...
            Vec3f exitant = photon.dir()*(-1.f);
            Vec3f incident = ray.dir()*(-1.f);
            float costTheta  = exitant.dot(normal);
            float brdf = m->BRDF(incident, exitant, normal, x,photon.wavelength())*costTheta;
    ...
    CUDA_CALLABLE_MEMBER float radiance(Ray ray_in, float wavelength,random_kit_t &random,int rdepth) const{
        ...
        float caustics = causticRadiance(object, ray, x, wavelength,random);
        return
        emittance +
        indirect +
        direct +
